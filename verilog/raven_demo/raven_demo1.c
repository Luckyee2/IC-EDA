...
void main()
{
	...
	// 1 standard (1x), 2 for DSPI (2x), 4 for QSPI (4x), 8 for DDR (8x)
	mode = 1;
	m = 1;
	c = 0;
	...
	while (1) {
		...
		print("mode = Latency test ");
		set_flash_latency(12);
		reg_spictrl = 0x805c0000;	// DSPI + CRM + latency
		m = 2;
	    }
	    else if (mode == 25) {
		print("mode = QSPI spd = 4x");
		reg_spictrl = 0x802c0000;	// QSPI
		m = 4;
	    }
	    else if (mode == 45) {
		print("mode = QSPI + CRM   ");
		...
		print("mode = QSPI + DDR   ");
		reg_spictrl = 0x806c0000;	// DDR + QSPI
		m = 8;
	    }
  	    else if (mode == 105) {
	    	//-----------------------------------------------------
	    	// Not sure why QSPI + DDR + CRM does not work on any
	    	// board even whe QSPI + DDR and QSPI + CRM both work.
		// Need to investigate this. . .
	    	//-----------------------------------------------------
  	    	print("mode = QSPI DDR CRM ");
  	    	reg_spictrl = 0x807c0000;	// DDR + QSPI + CRM
  	     	m = 8;
  	    }
	    else if (mode == 145) {
		print("mode = Single       ");
